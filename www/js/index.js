...
let { button, div: div$1, h2, input } = tags;
const styles$1 = {
    addBMButton: {
        background: '#D1C4E9',
...
function formatBM(bm) {
    return bm === '0' ? '' : bm;
}
const handleKeyPress = (session, index) => action(function (e) {
...
const bodyMotion = ({ index, session }) => input({
    id: 'body-motion-' + index,
    onkeypress: handleKeyPress(session, index),
    style: styles$1.bodyMotion,
    value: () => formatBM(session.bodyMotion[index].toString())
});
const sessionTA = (session) => div$1({ style: styles$1.taWrapper }, [
    h2({ style: styles$1.header }, ['Base TA']),
    input({
        oninput: session.changeTa,
        style: styles$1.taInput,
        type: 'number',
        value: () => session.baseTA.toString()
    }),
    button({
        onclick: session.addBodyMotion,
        style: styles$1.addBMButton
    }, ['Add Body Motion']),
        () => div$1({ style: styles$1.bodyMotionWrapper }, session.bodyMotion.map((_, index) => bodyMotion({ index, session })))
]);
...
const reduce = curry(function reduce(f, init, arr) {
    let acc = init;
...
const sum = curry(function sum(a, b) {
    return a + b;
});
const assign = Object.assign.bind(Object);

let { input: input$1, div: div$3, button: button$2, h2: h2$1 } = tags;
const styles$3 = {
    amPm: {
        background: '#D1C4E9',
        border: '1px solid white',
...
const changeHrs = (time) => action(function (e) {
...
const changeMins = (time) => action(function (e) {
    const parsed = parseInt(e.target.value, 10);
...
const toggleAm = (time) => action(function () {
    time.am = !time.am;
});
const timeEditor = ({ time, label }) => {
    const hrsProps = {
        max: 12,
        min: 0,
    ...
    return div$3({ style: styles$3.mainWrapper }, [
        h2$1({ style: styles$3.h2 }, [label]),
        div$3({ style: styles$3.inputWrapper }, [
            input$1(hrsProps),
            input$1(minsProps),
            button$2(amPmProps, [() => time.am ? 'AM' : 'PM'])
        ])
    ]);
};

let { div: div$2, button: button$1 } = tags;
const styles$2 = {
    addBreak: {
        background: '#D1C4E9',
        border: '1px solid white',
...
const breakItem = (store) => div$2({ style: styles$2.breakItem }, [
    timeEditor({ label: 'start', time: store.start }),
    timeEditor({ label: 'end', time: store.end })
]);
const breaks = (store) => () => div$2({ style: styles$2.breaksWrapper }, [
    button$1({ onclick: store.addBreak, style: styles$2.addBreak }, ['Add a break']),
    ...store.breaks.map(breakItem)
]);
const sessionTime = (store) => div$2({ style: styles$2.sessionWrapper }, [
    timeEditor({ label: 'start', time: store.start }),
    breaks(store),
    timeEditor({ label: 'end', time: store.end })
]);
...
class Break {
    constructor() {
        this.start = { am: true, hrs: 0, mins: 0 };
        this.end = { am: true, hrs: 0, mins: 0 };
    }
    get duration() {
...
class Session {
    ...
    get totalBreaks() {
        return reduce((acc, v) => sum(acc, v.duration), 0, this.breaks);
    }
    get totalTime() {
...
const total = (session) => div({ style: styles.totalWrapper }, [
